    ...
    <div className="flex flex-col gap-6">
      ...
      >
        <input
          ...
          className="flex-1 rounded border border-white/[0.08] bg-[#2a3040]/90 px-4 py-2.5 text-sm text-white/90 placeholder-white/30 backdrop-blur-md outline-none transition-colors focus:border-white/20"
        />
        <button
          type="submit"
          className="rounded border border-white/[0.08] bg-[#2a3040]/90 px-4 py-2.5 text-xs tracking-widest text-white/70 uppercase backdrop-blur-md transition-all hover:border-white/20 hover:bg-[#232b3d]/90"
        >
          Add
        </button>
      </form>
      ...
      {visibleSuggestions.length > 0 && (
        <div className="flex flex-col gap-2">
          ...
          {visibleSuggestions.map((suggestion) => (
            <div
              key={suggestion.id}
              className="flex items-center justify-between rounded border border-white/[0.06] bg-[#2a3040]/80 px-4 py-3 backdrop-blur-md"
            >
      ...
      {activeTodos.length > 0 && (
        <div className="flex flex-col gap-2">
          {activeTodos.map((todo) => (
            <div
              key={todo.id}
              className="flex items-center justify-between rounded border border-white/[0.08] bg-[#2a3040]/90 px-4 py-3 backdrop-blur-md transition-all hover:border-white/20"
            >
              <div className="flex items-center gap-3">
                <button
      ...
      {completedTodos.length > 0 && (
        <div className="flex flex-col gap-2">
          ...
          {completedTodos.map((todo) => (
            <div
              key={todo.id}
              className="flex items-center justify-between rounded border border-white/[0.05] bg-[#2a3040]/70 px-4 py-3"
            >
              <div className="flex items-center gap-3">
                <button
                  onClick={() => toggleTodo(todo.id)}
      ...
      {activeTodos.length === 0 && completedTodos.length === 0 && visibleSuggestions.length === 0 && (
        <div className="rounded border border-white/[0.05] bg-[#2a3040]/70 p-6 text-center">
